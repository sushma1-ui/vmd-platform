...
function children(n: Node): string { return (n.children ?? []).map(node).join(''); }
function node(n: Node): string {
  switch (n.type) {
    case 'text': return text(n);
    case 'paragraph': return `<p>${children(n)}</p>`;
    case 'heading': return `<${n.tag}>${children(n)}</${n.tag}>`;
    case 'list': return `<${n.listType === 'number' ? 'ol' : 'ul'}>${children(n)}</${n.listType === 'number' ? 'ol' : 'ul'}>`;
    case 'listitem': return `<li>${children(n)}</li>`;
    case 'link': return `<a href="${esc(n.fields?.url ?? '#')}">${children(n)}</a>`;
    case 'quote': return `<blockquote>${children(n)}</blockquote>`;
