...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  const underLimit = await rateLimit(`so:${clientAddress}`, 5, 60, { url: env.UPSTASH_REDIS_REST_URL, token: env.UPSTASH_REDIS_REST_TOKEN });
  if (!underLimit) return json({ ok: false, error: 'Too many requests' }, 429);

  const source = 'second-opinion' as const;
  const lead = { source, score: scoreLead(source), firstName: parsed.data.firstName, email: parsed.data.email, mobile: parsed.data.mobile, message: parsed.data.message, healthCheck: parsed.data };
  ...
  if (env.POSTMARK_SERVER_TOKEN && env.POSTMARK_FROM_EMAIL) {
    ...
    await sendTransactional({ to: parsed.data.email, template: 'second-opinion-received', model: parsed.data }, { serverToken: env.POSTMARK_SERVER_TOKEN, from: env.POSTMARK_FROM_EMAIL }).catch(() => {});
  }
  return json({ ok: true });
};
function fieldErrs(issues: { path: (string | number)[]; message: string }[]) { const o: Record<string, string> = {}; for (const i of issues) o[i.path.join('.')] = i.message; return o; }
