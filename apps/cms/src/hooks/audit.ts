 ...
 */
export const auditAfterChange: CollectionAfterChangeHook = async ({ req, operation, collection, doc }) => {
...
export const auditAfterDelete: CollectionAfterDeleteHook = async ({ req, collection, id }) => {
  ...
  await req.payload.create({
    collection: 'audit-log',
    data: { action: 'delete', collectionSlug: collection.slug, docId: String(id), user: req.user?.id },
    overrideAccess: true,
  });
};
