...
export interface UpstashConfig { url?: string; token?: string }
export async function rateLimit(key: string, limit: number, windowSec: number, cfg: UpstashConfig): Promise<boolean> {
  ...
  try {
    ...
    if (result === 1) await fetch(`${cfg.url}/expire/${encodeURIComponent(key)}/${windowSec}`, { headers });
    return result <= limit;
  } catch {
