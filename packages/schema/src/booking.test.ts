...
test('consultationRequest requires a mobile', () => {
  const r = consultationRequest.safeParse({ type: 'online', firstName: 'A', email: 'a@example.com' });
  assert.equal(r.success, false);
});
test('consultationRequest defaults timezone to Perth', () => {
  const r = consultationRequest.parse({ type: 'online', firstName: 'A', email: 'a@example.com', mobile: '+61400000000' });
